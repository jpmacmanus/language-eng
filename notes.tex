...
\begin{document}
  ...
  \noindent A domain-specific language (DSL) is a language that has been crafted with a
  ...
  \noindent and so on. An embedded DSL (EDSL) is defined within another host language. The
  ...
  \noindent Where $\llbracket \rrbracket$ are denotational brackets. We use them to ascribe a semantics.

  \[ \llbracket 3+5\rrbracket = \llbracket 3\rrbracket + \llbracket 5 \rrbracket = 3+5 = 8 \]

  \noindent We can model this using a deep embedding in Haskell with the following code:
  ...
  \noindent So instead of $\llbracket 3 + 5 \rrbracket$, we can now write
  ...
  \noindent Our example is now written as:
...
\end{remark}
  ...
  \noindent The first step isn't too hard, we simply get:
  ...
  \noindent Let's interpet the circuit language by stipulating that the semantics of a term is
  ...
  \noindent We can have multiple semantics easily by supplying more evaluation functions. For
  ...
  \noindent Shallow is problemaic because it is hard to add a different semantics. In order
  ...
  \noindent Here we want to extend the syntax by adding a new operation for multiplication,
  ...
  \noindent Again, we want to extend the semantics without modifying the code. (Though in this
  ...
  \noindent How do we generalise this? Our first step is to deconstruct the type of lists to expose its generic structure.
  ...
  \noindent We remove recursion from this data type, and mark it was a parameter \lstinline{k},
  ...
  \noindent Next, we make the recursive paramter something we can change programatically by
  ...
  \noindent We now need to derive a type that gives us the fixed point of data. This is defined
  as follows:

  \begin{lstlisting}
      data Fix f = In (f (Fix f)) \end{lstlisting}

  \noindent This datatype allows us to generalise all recursive data types (except mutually
  ...
  \noindent We say that \lstinline{List a} and \lstinline{_List_ a} are isomorphic when composing the above functions
  ...
  \noindent Some examples of values of type \lstinline{Fix (_List_ a)} are:

  \begin{lstlisting}
      In _Empty_ :: Fix (_List_ a)
  \end{lstlisting}

  \noindent (Note that the type of \lstinline{In} is \lstinline{In :: f (Fix f) -> Fix f}
  so the example above is where \lstinline{f} is \lstinline{_List_ a}.)

  \begin{lstlisting}
      In (_Cons_ 5 (In _Empty_))
  \end{lstlisting}

  \noindent and for two elements we have

  \begin{lstlisting}
      In (_Cons_ 6 (In (_Cons_ 7 (In _Empty_))))
  \end{lstlisting}

  \noindent So now we have enough to finish a definition of \lstinline{fromList}.

  \begin{lstlisting}
      fromList :: List a -> Fix (_List_ a)
      fromList Empty = In EmmptyF
      fromList (Cons x xs) = In (_Cons_ x (fromList xs))
  \end{lstlisting}
  ...
  \noindent Functions of that type correspond to replacing the constructors of \lstinline{_List_ a} with functions \lstinline{k}
  ...
  \noindent The function \lstinline{inop} is the opposite of \lstinline{In}. We define it by the following:
  ...
  \noindent So finally we can write the function cata by chasing the arrows of this square:
  ...
  \noindent An alternative and equivalent definition is:
  ...
  \begin{lstlisting}
      ...
      toList' cata alg
          where
              alg :: _List_ a [a] -> [a]
              alg _Empty_ = []
              alg _Cons_ x xs = x : xs
  \end{lstlisting}

  \noindent We can also define a function that returns the length of a \lstinline{Fix (_List_ a)}.
  ...
  \noindent Here is an example of evaluation.
  ...
  \noindent Now another exmaple, of summing a list.
  ...
  \noindent So the number 3 is written $S(S(S (0)))$. Our goal is to create a
  ...
  \noindent Step 2: Define a functor instance for \lstinline{_Peano_}.
  ...
  \noindent Step 3: Write functions using \lstinline{cata}.
  ...
  \noindent Now we can define a doubling function.
  ...
  \noindent We then learnt to extract the signature functor for this by locating recursive calls:
  ...
  \noindent The \lstinline{Fix _Expr_} datatype is essentially \lstinline{Expr}. Suppose we want to add multiplication
  ...
  \noindent This takes two functors and makes the functor \lstinline{(f:+:g)}. It introduces these
  constructors:
  ...
  \noindent The functor instance is defined as follows
  ...
  \noindent Now we are ready to define the signature functor for multiplication:

  \begin{lstlisting}
      data _Mul_ k = _Mul_ k k
  \end{lstlisting}

  \noindent This is the datatype constructor:

  \begin{lstlisting}
      _Mul_ :: k -> k -> _Mul_ k
  \end{lstlisting}

  \noindent We define its functor instance:

  \begin{lstlisting}
      instance Functor _Mul_ where
          fmap f (Mul x y) = _Mul_ (f x) (f y)
  \end{lstlisting}

  \noindent Finally, we can put the \lstinline{_Expr_} and \lstinline{_Mul_} languages together, to have a language with
  ...
  \noindent This is essentially the same as describing the following datatype, but in a
  compositional way:
  ...
  \noindent to reduce a \lstinline{Fix (_Expr_ :+: _Mul_)} type to  \lstinline{b}. To do this in a composotional way,
  ...
  \noindent So now, we can give a semantics to the language \lstinline{Fix (Expr:+:Mul)} by defining algebras.
  ...
  \begin{lstlisting}
      ...
      data _Add_ k = _Add_ k k
  \end{lstlisting}

  \noindent After defining functor instances, we can define functor instances for:

  \begin{lstlisting}
      Fix (_Val_ :+: _Add_ :+: _Int_)
  \end{lstlisting}
  ...
  BNF is a language used to expresss the shape of grammars. It was invented in around
  1958 for the expression of the Algol programming language. A BNF statement is made
  up of:

  \begin{itemize}
    \item \lstinline{emp_bnf} represents empty strings.
...
\begin{lstlisting}
    A ::= A $\alpha_1$ | $\cdots$ | A $\alpha_n$ | $\beta_1$ | $\cdots$ | $\beta_m$
\end{lstlisting}
...
\begin{lstlisting}
    ...
    A' ::= $\alpha_1$ A' | $\cdots$ | $\alpha_n$ A' | emp_bnf
\end{lstlisting}
...
\noindent To represent nondeterminism and alternation, we have these diagrams:
...
\noindent We can imagine that \lstinline{State s = Put s :+: Get s}:
...
\noindent To draw the operation \lstinline{Put}, we have the following:
...
\noindent However, with \lstinline{s = Int}, we have \lstinline{Free (Get Int) a}:
...
\noindent These syntax trees all correspond to values of type \lstinline{Free (State s) a}.
They can be cumbersome to work with because we must wrap everything in \lstinline{Op}.

\begin{lstlisting}
    Op (Put False (Op (Get (\s -> $\ldots$))))
\end{lstlisting}

\noindent We can avoid this by introducing smart constructors for \lstinline{Put} and
\lstinline{Get}:
...
\noindent And with these tools, we can simply substitute

\begin{lstlisting}
    put False >> get
\end{lstlisting}

\noindent and this produces the same tree as above.
